...
### 安全
- 堵住接受邀请的接管路径：未验证邮箱的账号可以登录（AUTH-03），他人可以抢先用被邀请的邮箱注册并设置密码与二次验证；此前“已有账号时忽略密码”会让该账号成为管理员。

能否在线执行：可以，不涉及 proto；控制面对应的迁移与实现在 panel 的 M1-02 中完成。

## v0.6.1（客户端安装包发布签名的 `key_id` 格式）

相对 v0.6.0。依据 workspace spec/02 CONV-30（新增客户端安装包发布签名）、spec/30 接口表，以及评审记录 `review/release-keyid-2026-09-25.md`。只修改描述、示例与响应字段的取值约束，oasdiff 只报告 1 条 info（`response-property-pattern-added`），非破坏性，按补丁版本发布；最低契约版本仍为 v0.6.0。

### 新增
- 无。

### 变更
- `Release.key_id`（`GET /v1/releases/latest`）增加描述与模式：1–255 的十进制字符串，与 `SignedConfig.key_id` 相同（CONV-30）。迁移：接口尚未实现，无兼容影响。
- `Release.signature` 增加描述：Ed25519 签名，64 字节，标准 base64 编码；密钥由发布流程离线保管，不在控制面中；签名输入在自研客户端立项时比照 spec/40 DEP-09 定义（workspace `backlog/M5.md`）。

### 修复
- `getLatestRelease` 示例：`key_id` 由 `rel-2026-01` 改为 `'1'`；`signature` 改为与 Ed25519 签名等长的 base64 值（此前的示例形似 ECDSA DER 编码）。
- 两份 OpenAPI 的 `info.version` 改为 0.6.1。

### 安全
- 无。

能否在线执行：可以，不涉及 proto 与数据库，控制面与前端无需修改。
...
- `SettingsUpdate.features` 按键合并，只修改提交的模块（与 `min_version` 的整体替换不同）；`features` 本身为 `null` 或非对象时返回 400 `invalid_request`（`invalid_format`，`field` 为 `features`）；某键的值为 `null` 或非布尔、模块名未知时返回 400 `invalid_request`（`invalid_format`，`field` 如 `features.support`）；开启未实现的模块返回 400 `invalid_request`（`not_allowed`），提交 `false` 始终允许（CONV-16）。迁移：设置管理接口尚未实现（M1-09），无已有调用方；后台只提交要修改的模块，并按构建时的模块清单置灰未实现的模块。
...
### 安全
- `issued_at` 严格递增：此前同一秒内的两次修改会得到相同的 `issued_at`，按“不早于”规则客户端会接受被替换的旧签名文档；副本时钟偏差也可能写入更早的值，使客户端长期拒绝新文档。已知且接受的例外：OPS-08 的模块屏蔽导致的有效值变化不写 `issued_at`，可能出现 `issued_at` 相同而 `features` 不同的两份文档，客户端都会接受；严格递增不等于“`issued_at` 相同则内容相同”。
- 批准设备授权与批准扫码登录需要重新验证：短期被盗的会话不能借批准操作换成攻击者设备上 90 天有效、不随原会话吊销的会话。

能否在线执行：可以，不涉及 proto 与数据库。控制面与管理后台、用户中心嵌入同一二进制，同时升级；回滚到已实现屏蔽规则的二进制（panel 跟进 v0.6.0 之后）时，新版本开启而该二进制未实现的模块按有效值规则下发 `false`；跟进之前的实现下发的是存储值。M1 阶段设置管理接口尚未实现、各模块都不能开启，只有直接改库才会出现这种情况。
